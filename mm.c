...
static void init_header_and_footer(void* header, void *footer, size_t size, int allocate){
    PUT(header, PACK(size,allocate));
    PUT(footer, PACK(size,allocate));
}
...
static void *coalesce(void *bp){
    previous_footer = FTRP(PREV_BLKP(bp));
    next_header = HDRP(NEXT_BLKP(bp));
    size_t size = GET_SIZE(HDRP(bp));
    if(is_previous_and_next_allocated(previous_footer, next_header)){
        return bp;
    }
    if(is_next_unallocated(previous_footer, next_haeder)){
    ...
    if(is_previous_unallocated(previous_footer, next_header)){
        size += GET_SIZE(previous_footer);
        PUT(FTRP(bp), PACK(size,0));
        PUT(HDRP(PREV_BLKP(bp)), PACK(size,0));
        return bp;
    }
    if(is_previous_and_next_unallocated()){
        size += (GET_SIZE(next_header) + GET_SIZE(previous_footer));
...
size_t define_adjust_size(size_t size){
    ...
    return DSIZE * ((size * ((size + (DSIZE) + (DSIZE - 1)) / DSIZE)));
}

void *find_with_first_fit(size_t adjust_size){
    start_address = (char*)mem_heap_lo() + DSIZE;
    while(1){
        if(start_address > mem_heap_hi()){
            return NULL;
        }
        if(GET_ALLOC(HDRP(start_address)) && GET_SIZE(HDRP(start_adrress)) == adjust_size){
            return (void *)bp;
        }
        start_address = NEXT_BLKP(start_address);
    }
}
void place(bp, adjust_size){
    init_header_and_footer(HDRP(bp), FTRP(bp), adjust_size, 1);
}
...
void mm_free(void *ptr){
    ...
    init_header_and_footer(HDRP(ptr), FTRP(ptr), 0, 0);
    coalesce(bp)
}
